...
function F_transverse_intersection(eq, x, scale_eig, c, V)
    ...
    project!(component(F, 4), component(P, m)(nothing, σ) - component(y, 1))

    F₅ = component(F, 5)

    for i ∈ 1:k-1
        yᵢ₊₁, yᵢ = component(y, i+1), component(y, i)
        project!(component(F₅, i), yᵢ(1, 0) + Integral(1, 0) * f(eq, yᵢ₊₁, yᵢ, γ_)- yᵢ₊₁)
    end
    Q, yₖ = c + V*θ, component(y, k)
    project!(component(F₅, k), yₖ(1, 0) + Integral(1, 0) * f(eq, Q, yₖ, γ_) - Q)
...
function DF_transverse_intersection(eq, x, c, V)
    ...
    project!(component(component(DF, 4, 3), :, m), Evaluation(nothing, σ))
    project!(Sequence(codomain(component(DF, 4, 4)), view(component(DF, 4, 4), :, 1)), differentiate(component(P, m), (0, 1))(nothing, σ))
    project!(component(component(DF, 4, 5), :, 1), -I)
    ...
    for i ∈ 1:k-1
        ...
        project!(component(D₅F₅, i, i+1), Integral(1, 0) * D₁f(eq, yᵢ₊₁, yᵢ, γ_) - I)
        project!(component(D₅F₅, i, i), Evaluation(1, 0) + Integral(1, 0) * D₂f(eq, yᵢ₊₁, yᵢ, γ_))
    end
    Q, yₖ = c + V*θ, component(y, k)
    mul!(component(D₁F₅, k, 1), Integral(1, 0), component(D₃f(eq, Q, yₖ, γ_), :, 1))
    project!(LinearOperator(domain(V), codomain(V), view(component(component(DF, 5, 4), k, :), :, 2+n:length(Ω))), Integral(1, 0) * D₁f(eq, Q, yₖ, γ_) * V - V)
    project!(component(D₅F₅, k, k), Evaluation(1, 0) + Integral(1, 0) * D₂f(eq, Q, yₖ, γ_))
